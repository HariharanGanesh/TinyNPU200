...
module axi4_lite_slave #(
    ...
    localparam [7:0] ADDR_TILE_COUNT      = 8'h74;
    localparam [7:0] ADDR_VERSION         = 8'h78;
    localparam [7:0] ADDR_FEATURE_FLAGS   = 8'h7C;
    localparam [7:0] ADDR_NUM_TILES       = 8'h80;
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            reg_layer_cfg1     <= 32'h0001_0001;   // Default: 1 in, 1 out
            reg_layer_cfg2     <= 32'h0010_0010;   // Default: 16x16 input
            reg_irq_ctrl       <= 32'd0;
            reg_m0             <= 32'd0;
            reg_n_shift        <= 32'd0;
            reg_bias           <= 32'd0;
            ...
            if (s_wvalid && s_wready) begin
                for (b = 0; b < DATA_WIDTH/8; b = b + 1) begin
                    if (s_wstrb[b]) begin
                        case (wr_addr[7:0])
                            ADDR_CTRL: begin
                                reg_ctrl[b*8 +: 8] <= s_wdata[b*8 +: 8];
                                $display("DUT_WRITE: Time %0t | ADDR_CTRL | b=%0d | wdata=%h", $time, b, s_wdata[b*8 +: 8]);
                            end
